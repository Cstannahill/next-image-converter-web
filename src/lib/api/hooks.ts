...
export function createApiHooks(client: ImageManipulationApiClient) {
  ...
  function useConvertImageConvertPostMutation() {
    const [data, setData] = useState<unknown | void | void | void | undefined>(
      undefined
    );
    const [error, setError] = useState<ApiClientError | undefined>(undefined);
    const [loading, setLoading] = useState(false);
    const mutate = useCallback(
      async (
        payload: { body: BodyConvertImageConvertPost },
        requestInit?: RequestInit
      ) => {
        setLoading(true);
        setError(undefined);
        try {
          const result = await client.convertImageConvertPost(
            payload.body,
            requestInit
          );
          setData(result);
  ...
  function useBatchConvertImagesConvertBatchPostMutation() {
    const [data, setData] = useState<
      unknown | void | void | HTTPValidationError | undefined
    >(undefined);
    const [error, setError] = useState<ApiClientError | undefined>(undefined);
    const [loading, setLoading] = useState(false);
    const mutate = useCallback(
      async (
        payload: { body: BodyBatchConvertImagesConvertBatchPost },
        requestInit?: RequestInit
      ) => {
        setLoading(true);
        setError(undefined);
        try {
          const result = await client.batchConvertImagesConvertBatchPost(
            payload.body,
            requestInit
          );
          setData(result);
  ...
  return {
    ...
    useConvertImageConvertPostMutation,
    useBatchConvertImagesConvertBatchPostMutation,
    useListFormatsConvertFormatsGetQuery,
    useRootGetQuery,
  };
}
